  ...
  try {
    let [, pid, stdin, stdout, stderr] = GLib.spawn_async_with_pipes(
      ...
      [
        ...
        "bash",
        "-c",
        `while IFS=$'\\n' read -r line; do bash -c "$line"; echo "EXIT_STATUS_OUTPUT $?";done`,
      ],
    ...
    return [
      async function runCommandLine(command_line) {
        ...
        await writeLine(
          stdinStream,
          command_line.replaceAll("\\", "\\\\").replaceAll('"', '\\"')
        );
        const lines = [];
        const readUntilExitStatus = async () => {
          const line = await readLine(stdoutStream);
          if (line.startswith("EXIT_STATUS_OUTPUT ")) {
            return [parseInt(line.split(" ")[1]), lines];
          }
          lines.push(line);
          return readUntilExitStatus();
        };
        prevCmdLinePromise = readUntilExitStatus();
...
var RootMode = class {
  ...
  constructor(infoCallback, rootModeToggle) {
    this._cancel = null;
    ...
    this._rtcwakeVersion = null;
  }

  _updateInfoLoop() {
    this._infoTimerId = new Promise((resolve, _) => {
      Promise.all([shutdownInfo(), this._wakeInfo()]).then(
        ([info, wakeInfo]) => {
  ...
  async wake(minutes) {
    return this.runCommandLine(
      `rtcwake --date +${minutes}min --mode on &\nPID=$!; sleep 0.1; kill $PID`
    );
  }

  async wakeCancel() {
    return this.runCommandLine(`rtcwake --mode disable`);
  }

  async _wakeInfo() {
    let deadline = -1;
    if (this._cancel !== null) {
      if (this._rtcwakeVersion === null) {
        this._rtcwakeVersion = await this.runCommandLine(
          "rtcwake --version",
          false
        ).catch(() => false);
        logDebug("rtcwake Available: " + this._rtcwakeVersion);
      }
      if (this._rtcwakeVersion) {
        // update wake timestamp info
        deadline = await this.runCommandLine(
          `WAKE=$(rtcwake --mode show | tail -c +12); if [ ! -z "$WAKE" ]; then date +%s -d "$WAKE"; else echo -1;fi`,
          false
        )
          .then(parseInt)
          .catch(() => -1);
  ...
  async _maybeTryWithoutRoot(cmd) {
    ...
    if (this._requiresRootCmd.includes(name)) {
      ...
      return execCheck(cmd, null, false).catch(() => {
  ...
  async runCommandLine(command_line, acquireRoot = true) {
    if (!acquireRoot && this._cancel === null) {
      throw new Error(`Root not yet aquired!`);
    }
    this._startRootProc();
