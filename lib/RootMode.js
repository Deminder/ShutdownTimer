...
var RootMode = class {
  ...
  async wake(minutes) {
    ...
    return this._maybeTryWithoutRoot([
      "sh",
      "-c",
      `echo ${timestamp} > /sys/class/rtc/${this._rtc}/wakealarm`,
    ]).catch(() => false);
  }
  ...
  async wakeCancel() {
    return this._maybeTryWithoutRoot([
      "sh",
      "-c",
      `echo 0 > /sys/class/rtc/${this._rtc}/wakealarm`,
    ]).catch(() => false);
  }
  ...
  async _maybeTryWithoutRoot(cmd) {
    ...
    if (this._requiresRootCmd.includes(command_line)) {
      return this.runCommandLine(command_line);
    } else {
      return execCheck(cmd, null, false).catch(() => {
        this._requiresRootCmd.push(command_line);
        return this.runCommandLine(command_line);
      });
    }
  }
