  ...
  class ShutdownTimer extends PopupMenu.PopupSubMenuMenuItem {
    ...
    _onModeChange() {
      ...
      ACTIONS.maybeStopRootModeProtection(this.internalScheduleInfo, true)
        ...
        .then(() => ACTIONS.maybeStartRootModeProtection(this.internalScheduleInfo));
    }
    ...
    _updateShutdownInfo(checkRunning = false) {
    ...
    _updateWakeModeItem() {
      ...
      this.wakeModeItem.label.text =
        modeLabel("wake") +
        ` ${hoursStr}${minutes % 60} ${_n("minute", "minutes", Math.abs(minutes))}`;
    }
