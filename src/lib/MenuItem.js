...
var ShutdownTimerItem = GObject.registerClass({
  Properties: {
    'shutdown-text': GObject.ParamSpec.string('shutdown-text', '', '',
      GObject.ParamFlags.READWRITE,
      ''),
    'indicator-icon-name': GObject.ParamSpec.string('indicator-icon-name', '', '',
      GObject.ParamFlags.READWRITE,
      'go-down-symbolic'),
    'indicator-show': GObject.ParamSpec.boolean('indicator-show', '', '',
      GObject.ParamFlags.READWRITE,
      false),
  },
}, class ShutdownTimerItem extends QuickMenuToggle {
  _init() {
    const gicon = Gio.icon_new_for_string(Me.path + '/icons/shutdown-timer-symbolic.svg');
    super._init({
      //canFocus: true,
  ...
  updateShutdownInfo() {
    let shutdownLabel;
    let shutdown_text = '';
    let indicator_icon_name = 'go-down-symbolic';
    if (this.internalScheduleInfo.scheduled && this.checkRunning) {
      ...
      indicator_icon_name = 'go-bottom-symbolic';
    } else {
      ...
      if (info.scheduled) {
        shutdown_text = durationString(info.secondsLeft);
      }
    }
    this.set({
      checked: this.internalScheduleInfo.scheduled,
      indicator_show: this.internalScheduleInfo.scheduled || this.externalScheduleInfo.scheduled,
      shutdown_text,
      indicator_icon_name,
    });
    this.menu.setHeader(this.shutdownTimerIcon, _('Shutdown Timer'), [
      shutdownLabel,
...
var ShutdownTimerIndicator = GObject.registerClass(
  class ShutdownTimerIndicator extends SystemIndicator {
    _init() {
      ...
      this._shutdownTimerItem.bind_property('indicator-icon-name',
        this._indicator, 'icon-name',
        GObject.BindingFlags.SYNC_CREATE
      );

      this._shutdownTimerItem.bind_property('indicator-show',
        this._indicator, 'visible',
        GObject.BindingFlags.SYNC_CREATE
      );

      this._shutdownTimerItem.bind_property('shutdown-text',
        this._scheduleLabel, 'text',
        GObject.BindingFlags.SYNC_CREATE
      );

      this._shutdownTimerItem.bind_property_full('shutdown-text',
        this._scheduleLabel, 'visible',
        GObject.BindingFlags.SYNC_CREATE,
        (__, text) => [true, !!text], null
      );
